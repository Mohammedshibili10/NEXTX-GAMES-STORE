...
export default function Newrelease() {
  ...
  return (
    <div className='text-white px-5 lg:px-22 py-20'>
      <div className='flex justify-between '>

        <h1 className='text-[26px] font-bold tracking-[1.5%] '>New Release</h1>
        <div className='flex gap-2'>
          <IoArrowBackCircleOutline onClick={prevSlide} className='w-12 h-12' />
          <IoArrowForwardCircleOutline onClick={nextSlide} className='w-12 h-12' />
        </div>
      </div>
      <div className='overflow-hidden'>
      <div className=' mt-12 flex transition-transform duration-700 ease-in-out   'style={{ transform: `translateX(-${current * 100}%)` }}>
        {slides.map((slide,index) => (
          <div key={index} className='min-w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2  gap-15 px-10 '>
            {slide.map((items,i)=>(
            <div key={i}>
